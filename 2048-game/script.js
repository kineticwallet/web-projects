document.addEventListener("DOMContentLoaded", () => {
    ...
    // Get the high score from local storage or set it to 0 if no found!
    ...
    function initializeGame() {
        board = [...Array(size)].map((value) => Array(size).fill(0));
        placeRandom();
        placeRandom();
        renderBoard();
    }
    ...
    function move(direction) {
        ...
        if (direction === "ArrowUp" || direction === "ArrowDown") {
            for (let j = 0; j < size; j++) {
                const column = [...Array(size)].map((_, index) => board[index][j]);
    ...
    function checkGameOver() {
        for (let i = 0; i < size; i++) {
            for (let j = 0; j < size; j++) {
                if (board[i][j] === 0) {
                    return; // There is a empty cell, so the game is not over!
                }
